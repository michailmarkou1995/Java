...
import { AppointmentInline } from '../classes/AppointmentInline';
...
export class SaveExportComponent implements OnInit {
  ...
  recordsTotal!: number;
  dtOptions1: any = {}; //<-no error hitting but above YES 
...
////this.patientService = //get doctor ID 
	  // var results = new Patient(this.username, this.first_name, this.last_name, this.email, this.phone, this.dateOfBirth, this.password);
  ...
  deleteUserPrompt(name: string, id: any) {
	...
	editUserPrompt(contentDoctor: any, username: string, app_id:string) {
		this.patientService.fetchPatient(username, app_id).subscribe(resp => {
			...
			//this.testx[0].prescription="lol";
			//diagnosed[0].prescription="null";
			this.patient_diagnosed=diagnosed[0].presription;
			this.prescriptions_directions=diagnosed[0].diagnosed;
