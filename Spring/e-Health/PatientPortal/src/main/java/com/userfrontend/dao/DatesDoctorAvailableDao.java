...
public interface DatesDoctorAvailableDao extends CrudRepository<DatesDoctorAvailable, Long> {
	 ...
	 List<Appointment> findDoctorInline(@Param("id") Long id);
	 ...
	 public List<DatesDoctorAvailable> FindAllNotWithDescriptionQuery(@Param("patient_id") long patient_id);
	 

	 @Query(value = "Select * FROM dates_doctor_available where doctor_account_doctor_id = :patient_id",  nativeQuery = true)
	 public List<DatesDoctorAvailable> findHimId(@Param("patient_id") long id);	 
}
